...
class RSSPoller:
    ...
    async def _fetch_article_list(self, fakeid: str, creds: Dict) -> List[Dict]:
        """
        Fetch articles with smart stop: fetch in batches of 10,
        stop when encountering articles already cached in DB.
        Fast-updating accounts get more fetched, slow ones stop early.
        Safety cap: up to 10 batches (100 articles) max.
        """
        headers = {
        ...
        begin = 0
        batch_size = 10
        max_batches = max(1, ARTICLES_PER_POLL // batch_size)  # safety cap
        ...
        async with httpx.AsyncClient(timeout=30.0) as client:
            for batch in range(max_batches):
                ...
                try:
                    ...
                    logger.error("Fetch error for %s at begin=%d: %s", fakeid[:8], begin, e)
                    break
                ...
                if base_resp.get("ret") != 0:
                    ...
                    if "login" in str(err_msg).lower() or ret_code == 200003:
                        auth_manager.mark_invalid(
                            "rss_poller",
                            f"fakeid={fakeid[:8]}, ret={ret_code}, msg={err_msg}",
                        )
                    logger.warning("WeChat API error for %s: ret=%s msg=%s",
                ...
                # Smart stop: check if any article in this batch already exists
                ...
                for a in articles_in_batch:
                    ...
                    if rss_store.article_exists(fakeid, link):
                        found_existing = True
                        logger.debug("Stop fetch for %s at begin=%d (found existing: %s)",
                                     fakeid[:8], begin, link[:50])
                        break
