...
@router.post("/rss/deep-poll", response_model=PollerStatusResponse,
             summary="深度轮询（滚动读取，遇已有即停）")
async def trigger_deep_poll():
    """
    深度轮询：对每个订阅公众号滚动拉取，**遇已有文章即停**。
    
    - 每批拉 10 篇，检查是否已缓存
    - 遇到已有文章立即停止，避免重复和无用请求
    - 快号(日更10+篇)多拉，慢号(周更)少拉，自适应
    - 安全上限 20 批 (200篇)，基本足够补 1-2 周数据
    """
    ...
    if not fakeids:
        return PollerStatusResponse(success=False, data={"message": "无订阅"})

    creds = auth_manager.get_credentials()
    if not creds or not creds.get("token") or not creds.get("cookie"):
        return PollerStatusResponse(success=False, data={"message": "未登录或登录已失效"})

    total_new = 0
    total_subs = len(fakeids)
    headers_tpl = {
    ...
    async with httpx.AsyncClient(timeout=30.0) as client:
        for fakeid in fakeids:
            try:
                ...
                begin = 0
                batch_size = 10
                max_batches = 20  # safety cap: up to 200 articles

                for batch in range(max_batches):
                    params = {
                        "sub": "list", "search_field": "null",
                        "begin": begin, "count": batch_size, "query": "",
                        "fakeid": fakeid, "type": "101_1",
                        "free_publish_type": 1, "sub_action": "list_ex",
                        "token": creds["token"], "lang": "zh_CN",
                        "f": "json", "ajax": 1,
                    }

                    resp = await client.get(
                        "https://mp.weixin.qq.com/cgi-bin/appmsgpublish",
                        params=params, headers=headers_tpl,
                    )
                    ...
                    if base_resp.get("ret") != 0:
                        ...
                        msg = base_resp.get("err_msg", "")
                        if ret == 200003:
                            logger.warning("Deep-poll: login invalid for %s", fakeid[:8])
                            break
                        # other ret codes just stop this subscription
                        break
                    ...
                    for item in publish_page.get("publish_list", []):
                        ...
                        if not isinstance(pub_info, dict):
                            continue
                        for a in pub_info.get("appmsgex", []):
                    ...
                    # Smart stop: check if any article already cached
                    found_existing = False
                    for a in articles_in_batch:
                        link = a.get("link", "")
                        if not link:
                            continue
                        if rss_store.article_exists(fakeid, link):
                            found_existing = True
                            logger.debug("Deep-poll: stop %s at batch %d (found existing)",
                                         fakeid[:8], batch)
                            break
                ...
                if all_articles:
                    ...
                    total_new += new_count
                    logger.info("Deep-poll: %d new articles for %s (fetched %d batches)",
                                new_count, fakeid[:8], (begin // batch_size) + 1)
                rss_store.update_last_poll(fakeid)

            except Exception as e:
                logger.error("Deep poll error for %s: %s", fakeid[:8], e)
            await asyncio.sleep(3)

    return PollerStatusResponse(
        success=True,
        data={
            "message": f"深度轮询完成: 检查 {total_subs} 个订阅, 新增 {total_new} 篇",
            "subscriptions_checked": total_subs,
